...
public class SteveRoutingTest {
    ...
    public void testHappyPath() {
        ...
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("steveRouting", variables);
        ...
        task = taskService.createTaskQuery()        		        	
                .singleResult();
        Assert.assertEquals("Next Task", task.getName());

        // complete next task with variables 
        Map<String, Object> taskVariables = new HashMap<String, Object>();
        taskVariables.put("destination", "phone");
        taskService.complete(task.getId(), taskVariables);

        // Service Task should be active
        task = taskService.createTaskQuery()        		        	
                .singleResult();
        Assert.assertEquals("Telephone interview", task.getName());
        ...
        taskVariables.put("destination", "next");
        taskService.complete(task.getId(), taskVariables);
        ...
        taskVariables = new HashMap<String, Object>();
        taskVariables.put("destination", null);
        taskService.complete(task.getId(), taskVariables);

        // Next Task should be active
        task = taskService.createTaskQuery()        		        	
                .singleResult();
        Assert.assertEquals("Final Task", task.getName());
